    ...
    for attempt in range(2):
        try:
            ...
            if attempt == 0:
                ...
                continue
    raise LLMError(f"Groq chat completion failed: {last_exc}")
...
async def _stream_once(
    ...
    body = {
        ...
        "messages": messages,
        "stream": True,
        "temperature": 0.3,  # low — this is a task-driven receptionist, not a creative chat
    }
    if tools:
        body["tools"] = tools
        body["tool_choice"] = "auto"

    # Accumulate partial tool-call argument strings by index — Groq streams
    # them token-by-token, same as OpenAI's function-calling protocol.
